...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.noteId) {
        this.note = this.$store.getters.getNoteById(this.noteId)
      } else {
        this.note = {
          noteId: random(),
          title: "",
    ...
    methods: {
       ...
       */
      deleteNote() {
        this.$store.commit("deleteNote", this.note.noteId)
        this.clearNote()
        this.$router.push("/")
      },
      /**
      ...
      async handleCancelEdit(noteId) {
        if (
          await confirm(
            "Do you realy want to cancel editing and exit to the main page?",
    ...
    async beforeRouteLeave(to, from, next) {
      if (
        await confirm(
          "Do you realy want to leave this page?",
          "All unsaved changes will be lost."
        )
      ) {
        this.clearNote()
